...
**Status:** Day 1 complete (2026-09-09). Project skeleton exists and runs against real Postgres. Day 2 (accounts) not started.
...
- **Day 2 — Accounts.** `django.contrib.auth`, register/login/logout templates, `base.html` layout, session cookies visible in devtools. *You learn:* Django's auth system, sessions vs. JWT (and why we chose sessions), CSRF, template inheritance.
- **Day 3 — Chat models & static shell.** `Conversation`/`Message` models + migration, a chat page that lists hardcoded/echoed messages (no LLM yet). *You learn:* Django ORM, migrations, `ModelForm`, `QuerySet`s in templates.
- **Day 4 — HTMX wiring.** Convert the echo chat into a real HTMX fragment-swap flow — still no LLM, just persist-and-redisplay. *You learn:* `hx-post`/`hx-target`/`hx-swap`, Django partial templates, why this beats a full-page-reload form.
