...
**Status:** Day 6 complete (2026-09-15). `SupervisorAgent` LLM-based route classification (with keyword fallback) wired into the real chat flow — verified end-to-end: `"Hi there!"` correctly classified `route="greeting"` (instant canned reply, no LLM call), `"Summarize the theory of relativity."` correctly classified `route="summary"` by the real LLM classifier (not the generic `"parallel"` default), both confirmed in Postgres. Day 7 (documents & ingestion) not started.
...
- ~~**Day 5 — First real LLM call.**~~ **DONE 2026-09-13.** Built `agents/llm_service.py`'s `LLMService` class — deliberately framework-agnostic (no Django imports at all): `__init__(self, model, host=None)` builds an `ollama.Client`, `generate(self, prompt)` sends a one-message chat request and returns just the reply string (`response["message"]["content"]`), no extra metadata leaked out. Wired it into `chat/views.py`'s `send_message`: `from agents.llm_service import LLMService` (absolute import, no leading dot, since `agents` is a sibling top-level package, not part of the `chat` app), instantiated with `settings.OLLAMA_MODEL`/`settings.OLLAMA_BASE_URL` (Django-specific config read in the view, then handed to `LLMService` as plain values — config-loading stays in the Django layer, never leaks into `agents/`), replacing the old `f"Echo: ..."` fake reply with a real `llm.generate(...)` call, `route="llm"` marking these as genuine (vs. Day 3/4's `route="echo"`). Verified for real: a full multi-turn conversation in the browser (correct Rayleigh-scattering explanation, correct "I have no physical body" self-awareness, no fabrication), `psql` confirming `route="llm"` on every new row, and real multi-second gaps between user/assistant timestamps proving the call is genuinely synchronous and blocking (not instant like the old echo) — a live demonstration of the Day 1 "synchronous Django views" decision, and exactly the pause Day 11+'s optional async upgrade exists to eventually remove. Two real bugs caught and fixed: `MessageForm(require_POST)`→`MessageForm(require.POST)`→`MessageForm(request.POST)` (autocomplete grabbing the wrong name twice before landing correctly) back on Day 4, and today a `responce`/`response` spelling typo. *You learned:* the framework-agnostic layer boundary in practice (not just in theory) — `LLMService` never imports Django, the view is solely responsible for translating Django config into plain constructor arguments; message-list shape for LLM chat APIs (same `{"role", "content"}` pattern from your LLM Engineering course, here via Ollama instead of OpenAI/Claude); Python class basics (`__init__`, `self`, instance attributes) and the ternary conditional expression; absolute vs. relative imports for sibling top-level packages.
- ~~**Day 6 — Supervisor & simple routing.**~~ **DONE 2026-09-15.** Built `agents/supervisor.py`'s `SupervisorAgent` — module-level constants (`VALID_ROUTES` set, `GREETING_KEYWORDS`/`SUMMARY_KEYWORDS`/`SEARCH_KEYWORDS` sets for the fallback path), `__init__(self, llm_service)` taking an already-constructed `LLMService` via dependency injection (no duplicate Django-settings reads), `decide_route(message)` wrapping `_classify_with_llm` in `try/except Exception` and falling back to `_classify_with_keywords` on any failure — including a deliberate `raise ValueError(...)` inside `_classify_with_llm` when the LLM returns something outside `VALID_ROUTES`, so "bad response" and "call failed" both funnel through the same fallback path. `_classify_with_keywords` uses `any(word in lowered for word in KEYWORDS)` generator-expression checks, defaulting to `"parallel"` per `PLAN.md`'s architecture (the safe "might need both search and summary" catch-all). Wired into `chat/views.py`'s `send_message`: one shared `LLMService` instance now passed to both `SupervisorAgent` (classification) and used directly (`generate`) for non-greeting replies; `route == "greeting"` short-circuits to a canned reply with zero LLM calls, every other route still falls through to a real `llm.generate()` call for now (differentiated `SearchAgent`/`SummaryAgent` behavior arrives Day 7-8) — `route=route` now records the *actual* classified route instead of Day 5's hardcoded `"llm"`. Verified for real: `"Hi there!"` → `route="greeting"`, instant, no LLM delay; `"Summarize the theory of relativity."` → `route="summary"`, correctly chosen by the real LLM classifier (not the keyword fallback's generic default) — both confirmed in `psql`. One real bug caught and fixed: an indentation slip that nested `assistant_message = conversation.messages.create(...)` and `return render(...)` inside the `else:` branch instead of after the whole `if/else` — meant a successful `greeting` reply would fall through to `return HttpResponse(status=400)` at the bottom, since nothing after `reply_text = "..."` ran in that branch. *You learned:* the agent/classification pattern (an LLM call whose job is choosing between fixed options, not generating free text); dependency injection (why `SupervisorAgent` takes an `LLMService` instead of building its own); `try/except Exception` graceful degradation as a deliberate boundary choice, not a blanket habit; generator expressions inside `any()`; the real difference between `if/elif/elif` (mutually exclusive, first match wins) and sequential `if/if/if` (every condition evaluated independently) — and why early `return`s made the two behave identically here.
...
claude --resume dc001d6e-882e-4b22-9b84-853a8b7827ed
